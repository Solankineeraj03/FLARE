...
def main():
    ...
    parser.add_argument('--seed', type=int, default=2025, help='Value for random seed') 
    ...
    for split in splits:
        ...
        orig_model.load_state_dict(torch.load('CIFAR10_teacher_32_model.pth'))
        ...
        unlabeled_dataloader = torch.utils.data.DataLoader(train_dataset, sampler=unlabeled_sampler, batch_size=args.batch_size, drop_last=True)

        FE, classifier = pretrain_models(args, FE, classifier, querry_dataloader, val_dataloader, logger)

        FE, classifier, decoder, discriminator, best_weights = train_models(args, orig_model, FE, classifier, decoder, discriminator, querry_dataloader, val_dataloader, unlabeled_dataloader, logger)

        FE.eval()
        classifier.eval()
        discriminator.eval()

        infer_sampler(FE, discriminator, querry_dataloader, args.device, logger)
        infer_sampler(FE, discriminator, unlabeled_dataloader, args.device, logger)
        ...
        torch.save(decoder.state_dict(), os.path.join(save_path, f'{args.dataset}_decoder_model_{split}.pth'))
        torch.save(discriminator.state_dict(), os.path.join(save_path, f'{args.dataset}_sampler_model_{split}.pth'))

        sampled_indices = sample_for_labeling(args, args.budget, FE, decoder, discriminator, unlabeled_dataloader)

        # plot_tsnee(split, train_dataset, current_indices, sampled_indices, FE)
        ...
        FE.load_state_dict(best_weights['FE'])
        ...
        torch.save(FE.state_dict(), os.path.join(save_path, f'{args.dataset}_fe_best_model_{split}.pth'))
        torch.save(classifier.state_dict(), os.path.join(save_path, f'{args.dataset}_classifier_best_model_{split}.pth'))
